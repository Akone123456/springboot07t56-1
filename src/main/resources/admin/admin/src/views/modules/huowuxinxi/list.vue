<template>
  <div class="main-content">
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                ...
                <el-table-column
                 ...
                 :header-align="contents.tableAlign"
                  ...
                  <template slot-scope="scope">
                    ...
                    <el-button v-if="scope.row.ispay!='已支付' && isAuth('huowuxinxi','支付') " type="text" icon="el-icon-edit" size="small" @click="payHandler(scope.row)">支付</el-button>
                  </template>
                </el-table-column>
                <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign" 
                    prop="fulladdress"
            ...
            <el-table-column width="300" :align="contents.tableAlign" 
               :header-align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('huowuxinxi','运单信息') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="yundanxinxiCrossAddOrUpdateHandler(scope.row,'cross','是','','')">{{ contents.tableBtnFont == 1?'运单信息':'' }}</el-button>
                <el-button v-if="isAuth('huowuxinxi','运单信息') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="yundanxinxiCrossAddOrUpdateHandler(scope.row,'cross','是','','')">{{ contents.tableBtnFont == 1?'运单信息':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('huowuxinxi','运单信息') && contents.tableBtnIcon == 0" type="success" size="mini" @click="yundanxinxiCrossAddOrUpdateHandler(scope.row,'cross','是','','')">{{ contents.tableBtnFont == 1?'运单信息':'' }}</el-button>
    ...
    <yundanxinxi-cross-add-or-update v-if="yundanxinxiCrossAddOrUpdateFlag" :parent="this" ref="yundanxinxiCrossaddOrUpdate"></yundanxinxi-cross-add-or-update>
...
import yundanxinxiCrossAddOrUpdate from "../yundanxinxi/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      yundanxinxiCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    yundanxinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
	if(crossOptAudit=='是'&&row.sfsh!='是') {
      ...
      this.yundanxinxiCrossAddOrUpdateFlag = true;
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.yundanxinxiCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.yundanxinxiCrossaddOrUpdate.init(row.id,type);
      });
    },
    payHandler(row){
      this.$storage.set('paytable','huowuxinxi');
